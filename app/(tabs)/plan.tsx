...
import {
  monthsToText, MonthlySnapshot, Debt, debtsEligibleForStrategy, runStrategy, DebtMonthSnapshot,
...
export default function PlanScreen() {
  ...
  const {
    activeResult, debts, selectedStrategy, payments, logPayment,
    extraPayment,
  } = useDebts();
  ...
  const interestSaved = useMemo(() => {
    if (eligibleDebts.length === 0) return 0;
    const minOnly = runStrategy(eligibleDebts, 0, selectedStrategy === "custom" ? "snowball" : selectedStrategy);
    return Math.max(0, minOnly.totalInterestPaid - activeResult.totalInterestPaid);
  }, [eligibleDebts, selectedStrategy, activeResult]);
