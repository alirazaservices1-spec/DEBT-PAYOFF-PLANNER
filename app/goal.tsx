...
function GoalResearchBlock({ C }: { C: ThemeColors }) {
  const stats = [
    { label: "Thinking about goals only", pct: "43%" },
    { label: "Writing goals down", pct: "61%" },
    { label: "Sharing the goal with a friend", pct: "64%" },
    { label: "Sending weekly progress reports", pct: "76%" },
  ] as const;
  const principles = [
    {
      title: "Encoding",
      body: "Writing engages your brain more deeply than thinking alone, improving memory and focus.",
    },
    {
      title: "External storage",
      body: "A written goal is a visible anchor that helps prevent “goal drift.”",
    },
    {
      title: "Endowment effect",
      body: "Putting a goal in writing helps you “own” it as part of your identity.",
    },
  ] as const;

  return (
    <View style={[styles.researchCard, { backgroundColor: C.surfaceSecondary, borderColor: C.border }]}>
      <Text style={[styles.researchEyebrow, { color: C.textSecondary }]}>Evidence-based motivation</Text>
      <Text style={[styles.researchTitle, { color: C.text }]}>
        Why writing your goal down works
      </Text>
      <Text style={[styles.researchBody, { color: C.textSecondary }]}>
        The most famous and widely cited work here is the{" "}
        <Text style={{ fontFamily: Fonts.bold, color: C.text }}>
          Dominican University Goal Setting Study (2007)
        </Text>
        , conducted by Dr. Gail Matthews. It provides strong evidence that writing down goals—and layering
        commitment—materially raises the odds of follow-through.
      </Text>

      <Text style={[styles.researchSectionHead, { color: C.text }]}>What the study found</Text>
      {stats.map((row) => (
        <View key={row.label} style={[styles.researchStatRow, { borderBottomColor: C.border }]}>
          <Text style={[styles.researchStatLabel, { color: C.text }]}>{row.label}</Text>
          <Text style={[styles.researchStatPct, { color: Colors.primary }]}>{row.pct}</Text>
        </View>
      ))}

      <Text style={[styles.researchHighlight, { color: C.text }]}>
        You&apos;re roughly <Text style={{ fontFamily: Fonts.extraBold, color: Colors.primary }}>42% more likely</Text> to
        achieve a goal just by writing it down. With accountability and progress check-ins—like the kind this app
        supports—that rises to <Text style={{ fontFamily: Fonts.extraBold, color: Colors.primary }}>76%</Text> in the
        study&apos;s strongest group.
      </Text>

      <Text style={[styles.researchSectionHead, { color: C.text, marginTop: 4 }]}>Why it works</Text>
      {principles.map((row) => (
        <View key={row.title} style={styles.researchBulletRow}>
          <Text style={styles.researchBullet}>•</Text>
          <Text style={[styles.researchBulletText, { color: C.textSecondary }]}>
            <Text style={{ fontFamily: Fonts.bold, color: C.text }}>{row.title}:</Text> {row.body}
          </Text>
        </View>
      ))}

      <Text style={[styles.researchSectionHead, { color: C.text }]}>Using this in DebtPath</Text>
      <Text style={[styles.researchBody, { color: C.textSecondary }]}>
        You can mirror the study&apos;s strongest habits here: keep your goal visible, update progress as you pay down
        debt, and use reminders or weekly rhythm to stay accountable—similar to the “report to a friend” effect in the
        research.
      </Text>

      <Text style={[styles.researchCitation, { color: C.textSecondary }]}>
        Source: Matthews, G. (2007). Dominican University of California goal-setting study — widely summarized in
        productivity and coaching literature; results above reflect commonly reported group outcomes from that work.
      </Text>
    </View>
  );
}
