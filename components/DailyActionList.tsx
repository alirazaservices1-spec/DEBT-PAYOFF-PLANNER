  ...
  return (
    <View style={[s.container, { backgroundColor: C.bg, borderColor: C.border }]}>
      ...
      <ActionItem 
        ...
        title="Saved $0.50 today"
        subtitle="Set aside in addition to your normal payment amount."
        xp={20}
        done={savedMoney}
...
function ActionItem({ icon, color, title, subtitle, xp, done, onPress, C }: any) {
  return (
    <Pressable 
      onPress={onPress} 
      disabled={done}
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        ...
        {subtitle ? (
          <Text style={[s.itemSubtitle, { color: C.textSec, textDecorationLine: done ? "line-through" : "none" }]}>
            {subtitle}
          </Text>
        ) : null}
      ...
      <View style={[s.checkbox, { marginTop: 2 }, done && { backgroundColor: Colors.amber, borderColor: Colors.amber }]}>
