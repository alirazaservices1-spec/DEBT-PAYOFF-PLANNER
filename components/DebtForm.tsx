...
export function DebtForm({ initial, onSave, onCancel, headerExtra, title }: Props) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const introParsed = !isTaxDebt && hasIntroRate ? parseMonthYearString(introEnds.trim()) : null;
      await onSave({
        name: name.trim(),
            ...
            >
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.taxPlanHint, { color: formMutedColor }]}>
                  Monthly payment, interest rate, and due date are optional unless you’re on an installment plan—then enter what the agency gave you. IRS underpayment interest is set by law (federal short-term rate + 3%) and can change each calendar quarter; it isn’t a negotiated “locked” rate like a car loan.
                </Text>
              </View>
              <Switch
